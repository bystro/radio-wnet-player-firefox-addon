...
function currentBroadcastInfo() {
  ...
  getFirstParagraph = "console.log(document.body.innerHTML);" +                        
                          "self.port.emit('loaded', document.body.innerHTML);"
  ...
  pageWorker.port.on("loaded", function(strCurrentBroadcastInfo) {
    ...
    require('sdk/timers').setTimeout(currentBroadcastInfo, 1000 * 30);   
...
function lastFavouriteBroadcasts() {
  ...
  pageWorker.port.on("loaded", function(strLastFavouriteBroadcasts) {
    //console.log(strLastFavouriteBroadcasts);    
